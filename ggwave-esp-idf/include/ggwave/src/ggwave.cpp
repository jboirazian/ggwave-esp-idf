...
#define ggprintf(...)
#else
#define ggprintf(...) ESP_LOGI("ggwave-lib", __VA_ARGS__)
#endif
...
double linear_interp(double first_number, double second_number,
                     double fraction) {
...
extern "C" void ggwave_setLogFile(void *fptr) {
...
extern "C" ggwave_Instance ggwave_init(ggwave_Parameters parameters) {
  for (ggwave_Instance id = 0; id < GGWAVE_MAX_INSTANCES; ++id) {
    if (g_instances[id] == nullptr) {
      g_instances[id] = new GGWave(
          {parameters.payloadLength, parameters.sampleRateInp,
           parameters.sampleRateOut, parameters.sampleRate,
           parameters.samplesPerFrame, parameters.soundMarkerThreshold,
           parameters.sampleFormatInp, parameters.sampleFormatOut,
  ...
  ggprintf("Failed to create GGWave instance - reached maximum number of "
           "instances (%d)\n",
...
extern "C" void ggwave_free(ggwave_Instance id) {
...
extern "C" int ggwave_encode(ggwave_Instance id, const void *payloadBuffer,
                             int payloadSize, ggwave_ProtocolId protocolId,
                             int volume, void *waveformBuffer, int query) {
  ...
  if (ggWave->init(payloadSize, (const char *)payloadBuffer, protocolId,
                   volume) == false) {
...
extern "C" int ggwave_decode(ggwave_Instance id, const void *waveformBuffer,
...
extern "C" int ggwave_ndecode(ggwave_Instance id, const void *waveformBuffer,
                              int waveformSize, void *payloadBuffer,
                              int payloadSize) {
  GGWave *ggWave = (GGWave *)g_instances[id];
...
extern "C" void ggwave_rxToggleProtocol(ggwave_ProtocolId protocolId,
                                        int state) {
  GGWave::Protocols::rx().toggle(protocolId, state != 0);
}

extern "C" void ggwave_txToggleProtocol(ggwave_ProtocolId protocolId,
                                        int state) {
  GGWave::Protocols::tx().toggle(protocolId, state != 0);
}

extern "C" void ggwave_rxProtocolSetFreqStart(ggwave_ProtocolId protocolId,
                                              int freqStart) {
  GGWave::Protocols::rx()[protocolId].freqStart = freqStart;
}

extern "C" void ggwave_txProtocolSetFreqStart(ggwave_ProtocolId protocolId,
                                              int freqStart) {
  GGWave::Protocols::tx()[protocolId].freqStart = freqStart;
}

extern "C" int ggwave_rxDurationFrames(ggwave_Instance id) {
  GGWave *ggWave = (GGWave *)g_instances[id];
...
// this achieves more homogeneous distribution of the sound energy across the
// spectrum
constexpr int kDSSMagicSize = 64;
const uint8_t kDSSMagic[kDSSMagicSize] PROGMEM = {
    0x96, 0x9f, 0xb4, 0xaf, 0x1b, 0x91, 0xde, 0xc5, 0x45, 0x75, 0xe8,
    0x2e, 0x0f, 0x32, 0x4a, 0x5f, 0xb4, 0x56, 0x95, 0xcb, 0x7f, 0x6a,
    0x54, 0x6a, 0x48, 0xf2, 0x0b, 0x7b, 0xcd, 0xfb, 0x93, 0x6d, 0x3c,
    0x77, 0x5e, 0xc3, 0x33, 0x47, 0xc0, 0xf1, 0x71, 0x32, 0x33, 0x27,
    0x35, 0x68, 0x47, 0x1f, 0x4e, 0xac, 0x23, 0x42, 0x5f, 0x00, 0x37,
    0xa4, 0x50, 0x6d, 0x48, 0x24, 0x91, 0x7c, 0xa1, 0x4e,
};
...
void FFT(float *f, int N, int *wi, float *wf) { rdft(N, 1, f, wi, wf); }
...
inline void addAmplitudeSmooth(const GGWave::Amplitude &src,
                               GGWave::Amplitude &dst, float scalar,
                               int startId, int finalId, int cycleMod,
                               int nPerCycle) {
...
template <typename T> void ggvector<T>::assign(const ggvector &other) {
  m_data = other.m_data;
  m_size = other.m_size;
}

template <typename T> void ggvector<T>::copy(const ggvector &other) {
  if (this == &other) {
    assert(false);
  }
...
template <typename T> void ggvector<T>::zero() {
  memset(m_data, 0, m_size * sizeof(T));
}

template <typename T> void ggvector<T>::zero(int n) {
...
template <typename T> void ggmatrix<T>::zero() {
...
//
// template <typename T>
// void ggalloc(std::vector<std::vector<T>> & v, int n, int m, void * buf, int &
// bufSize) {
//     if (buf == nullptr) {
...
GGWave::GGWave(const Parameters &parameters) { prepare(parameters); }
...
bool GGWave::prepare(const Parameters &parameters, bool allocate) {
  ...
  m_encodedDataOffset =
      parameters.payloadLength > 0 ? 0 : kDefaultEncodedDataOffset;
  ...
  m_needResampling =
      m_sampleRateInp != m_sampleRate || m_sampleRateOut != m_sampleRate;
  m_txOnlyTones =
...
bool GGWave::alloc(void *p, int &n) {
  const int maxLength =
      m_isFixedPayloadLength ? m_payloadLength : kMaxLengthVariable;
  const int totalLength = maxLength + getECCBytesForLength(maxLength);
  const int totalTxs = (totalLength + minBytesPerTx(Protocols::rx()) - 1) /
                       minBytesPerTx(Protocols::tx());

  if (totalLength > kMaxDataSize) {
    ggprintf("Error: total length %d (payload %d + ECC %d bytes) is too large "
             "( > %d)\n",
             totalLength, maxLength, getECCBytesForLength(maxLength),
             kMaxDataSize);
    return false;
  }
  ...
  if (m_isRxEnabled) {
    ...
    ::ggalloc(m_rx.amplitude,
              m_needResampling ? m_samplesPerFrame + 128 : m_samplesPerFrame, p,
              n);
    // min input sampling rate is 0.125*m_sampleRate:
    ::ggalloc(m_rx.amplitudeResampled,
              m_needResampling ? 8 * m_samplesPerFrame : m_samplesPerFrame, p,
              n);
    ::ggalloc(m_rx.amplitudeTmp,
              m_needResampling ? 8 * m_samplesPerFrame * m_sampleSizeInp
                               : m_samplesPerFrame * m_sampleSizeInp,
              p, n);

    ::ggalloc(m_rx.data, maxLength + 1, p,
              n); // extra byte for null-termination
    ...
    if (m_isFixedPayloadLength) {
      ...
      ::ggalloc(m_rx.spectrumHistoryFixed,
                totalTxs * maxFramesPerTx(Protocols::rx(), false),
                m_samplesPerFrame, p, n);
      ::ggalloc(m_rx.detectedBins, 2 * totalLength, p, n);
      ::ggalloc(m_rx.detectedTones, 2 * 16 * maxBytesPerTx(Protocols::rx()), p,
                n);
    } else {
      // variable payload length
      ::ggalloc(m_rx.amplitudeRecorded, kMaxRecordedFrames * m_samplesPerFrame,
                p, n);
      ::ggalloc(m_rx.amplitudeAverage, m_samplesPerFrame, p, n);
      ::ggalloc(m_rx.amplitudeHistory, kMaxSpectrumHistory, m_samplesPerFrame,
                p, n);
    }
  }
  ...
  if (m_isTxEnabled) {
    ...
    if (m_txOnlyTones == false) {
      ...
      ::ggalloc(m_tx.outputTmp,
                kMaxRecordedFrames * m_samplesPerFrame * m_sampleSizeOut, p, n);
      ::ggalloc(m_tx.outputI16, kMaxRecordedFrames * m_samplesPerFrame, p, n);
    }

    const int maxTones = m_isFixedPayloadLength ? maxTonesPerTx(Protocols::tx())
                                                : m_nBitsInMarker;

    ::ggalloc(m_tx.data, maxLength + 1, p, n); // first byte stores the length
    ::ggalloc(m_tx.dataBits, maxDataBits, p, n);
    ::ggalloc(m_tx.tones, maxTones * totalTxs + (maxTones > 1 ? totalTxs : 0),
              p, n);
  }
  ...
  // pre-allocate Reed-Solomon memory buffers
  {
    ...
    if (m_isFixedPayloadLength == false) {
      ::ggalloc(m_workRSLength,
                RS::ReedSolomon::getWorkSize_bytes(1, m_encodedDataOffset - 1),
                p, n);
    }
    ::ggalloc(m_workRSData,
              RS::ReedSolomon::getWorkSize_bytes(
                  maxLength, getECCBytesForLength(maxLength)),
              p, n);
  }
...
void GGWave::setLogFile(FILE *fptr) { g_fptr = fptr; }
...
bool GGWave::init(int dataSize, const char *dataBuffer, TxProtocolId protocolId,
                  const int volume) {
  if (dataSize < 0) {
  ...
  if (m_isTxEnabled) {
    ...
    if (dataSize > 0) {
      ...
      if (protocol.enabled == false) {
        ggprintf("Protocol %d is not enabled - make sure to enable it before "
...
uint32_t GGWave::encodeSize_samples() const {
  ...
  if (m_needResampling) {
    ...
    samplesPerFrameOut = m_resampler.resample(factor, m_samplesPerFrame,
                                              m_tx.output.data(), nullptr) +
                         1;
  }
  ...
  const int totalDataFrames =
      m_tx.protocol.extra *
      ((totalBytes + m_tx.protocol.bytesPerTx - 1) / m_tx.protocol.bytesPerTx) *
      m_tx.protocol.framesPerTx;

  return (m_nMarkerFrames + totalDataFrames + m_nMarkerFrames) *
         samplesPerFrameOut;
}
...
uint32_t GGWave::encode() {
  ...
  RS::ReedSolomon rsData =
      RS::ReedSolomon(m_tx.dataLength, nECCBytesPerTx, m_workRSData.data());
  rsData.Encode(m_tx.data.data() + 1,
                m_dataEncoded.data() + m_encodedDataOffset);
        ...
        for (int j = 0; j < m_tx.protocol.bytesPerTx; ++j) {
          if (m_tx.protocol.extra == 1) {
            ...
            if (dataOffset % m_tx.protocol.extra == 0) {
              uint8_t d =
                  m_dataEncoded[dataOffset / m_tx.protocol.extra + j] & 15;
              m_tx.dataBits[(2 * j + 0) * 16 + d] = 1;
            } else {
              uint8_t d =
                  m_dataEncoded[dataOffset / m_tx.protocol.extra + j] & 240;
  ...
  // compute Tx data
  {
    ...
    for (int k = 0; k < (int)m_tx.dataBits.size(); ++k) {
      ...
      for (int i = 0; i < m_samplesPerFrame; i++) {
        const double curi = i;
        m_tx.bit1Amplitude[k][i] =
            sin((2.0 * M_PI) * (curi * m_isamplesPerFrame) *
                    (freq * curIHzPerSample) +
                phaseOffset);
      }

      for (int i = 0; i < m_samplesPerFrame; i++) {
        const double curi = i;
        m_tx.bit0Amplitude[k][i] = sin(
            (2.0 * M_PI) * (curi * m_isamplesPerFrame) *
  ...
  while (m_tx.hasData) {
    ...
    if (frameId < m_nMarkerFrames) {
      ...
      for (int i = 0; i < m_nBitsInMarker; ++i) {
        if (i % 2 == 0) {
          ::addAmplitudeSmooth(m_tx.bit1Amplitude[i], m_tx.output,
                               m_tx.sendVolume, 0, m_samplesPerFrame, frameId,
                               m_nMarkerFrames);
        } else {
          ::addAmplitudeSmooth(m_tx.bit0Amplitude[i], m_tx.output,
                               m_tx.sendVolume, 0, m_samplesPerFrame, frameId,
                               m_nMarkerFrames);
        }
      }
      ...
      for (int k = 0; k < 2 * m_tx.protocol.bytesPerTx * 16; ++k) {
        ...
        ++nFreq;
        if (k % 2) {
          ::addAmplitudeSmooth(m_tx.bit0Amplitude[k / 2], m_tx.output,
                               m_tx.sendVolume, 0, m_samplesPerFrame,
                               cycleModMain, m_tx.protocol.framesPerTx);
        } else {
          ::addAmplitudeSmooth(m_tx.bit1Amplitude[k / 2], m_tx.output,
                               m_tx.sendVolume, 0, m_samplesPerFrame,
                               cycleModMain, m_tx.protocol.framesPerTx);
        }
      }
      ...
      for (int i = 0; i < m_nBitsInMarker; ++i) {
        if (i % 2 == 0) {
          addAmplitudeSmooth(m_tx.bit0Amplitude[i], m_tx.output,
                             m_tx.sendVolume, 0, m_samplesPerFrame, fId,
                             m_nMarkerFrames);
        } else {
          addAmplitudeSmooth(m_tx.bit1Amplitude[i], m_tx.output,
                             m_tx.sendVolume, 0, m_samplesPerFrame, fId,
    ...
    if (m_needResampling) {
      samplesPerFrameOut =
          m_resampler.resample(factor, m_samplesPerFrame, m_tx.output.data(),
                               m_tx.outputResampled.data());
    } else {
...
bool GGWave::decode(const void *data, uint32_t nBytes) {
  ...
  while (true) {
    ...
    if (m_needResampling) {
      ...
      nBytesNeeded =
          (m_resampler.resample(1.0f / factor, m_rx.samplesNeeded,
                                m_rx.amplitudeResampled.data(), nullptr) +
           4) *
          m_sampleSizeInp;
    }
    ...
    if (nBytesRecorded % m_sampleSizeInp != 0) {
      ggprintf("Failure during capture - provided bytes (%d) are not multiple "
               "of sample size (%d)\n",
    ...
    if (m_needResampling) {
      ...
      if (!m_rx.receiving &&
          m_resampler.nSamplesTotal() > 60.0f * factor * m_sampleRate) {
        m_resampler.reset();
      }

      int nSamplesResampled =
          offset + m_resampler.resample(factor, nSamplesRecorded,
                                        m_rx.amplitudeResampled.data(),
                                        m_rx.amplitude.data() + offset);
...
GGWave::SampleFormat GGWave::sampleFormatInp() const {
  return m_sampleFormatInp;
}
GGWave::SampleFormat GGWave::sampleFormatOut() const {
  return m_sampleFormatOut;
}
...
const GGWave::Tones GGWave::txTones() const {
  return {m_tx.tones.data(), m_tx.nTones};
}

bool GGWave::txHasData() const { return m_tx.hasData; }

bool GGWave::txTakeAmplitudeI16(AmplitudeI16 &dst) {
  if (m_tx.lastAmplitudeSize == 0)
    return false;
...
const GGWave::RxProtocols &GGWave::txProtocols() const {
  return m_tx.protocols;
}
...
const GGWave::RxProtocolId &GGWave::rxProtocolId() const {
  return m_rx.protocolId;
}
...
int GGWave::computeFFTR(const float *src, float *dst, int N, int *wi,
                        float *wf) {
  if (wi == nullptr)
    return 2 * N;
  if (wf == nullptr)
...
int GGWave::filter(ggwave_Filter filter, float *waveform, int N, float p0,
                   float p1, float *w) {
  if (w == nullptr) {
    switch (filter) {
  ...
  case GGWAVE_FILTER_FIRST_ORDER_HIGH_PASS: {
    for (int i = 0; i < N; i++) {
      float xn = waveform[i];
      float yn =
          w[0] * xn + w[1] * w[5] + w[2] * w[6] + w[3] * w[7] + w[4] * w[8];
      w[6] = w[5];
      w[5] = xn;
      w[8] = w[7];
      w[7] = yn;
...
int GGWave::Resampler::resample(float factor, int nSamples,
                                const float *samplesInp, float *samplesOut) {
  int idxInp = -1;
  int idxOut = 0;
  int notDone = 1;
  float data_in = 0.0f;
  ...
  while (notDone) {
    ...
    double temp1 = 0.0;
    int left_limit = m_state.timeNow - kWidth +
                     1; /* leftmost neighboring sample used for interp.*/
    int right_limit =
        m_state.timeNow +
        kWidth; /* rightmost leftmost neighboring sample used for interp.*/
    if (left_limit < 0)
      left_limit = 0;
    if (right_limit > m_state.nSamplesTotal + kWidth)
      right_limit = m_state.nSamplesTotal + kWidth;
    if (factor < 1.0) {
      for (int j = left_limit; j < right_limit; j++) {
        temp1 +=
            getData(j - m_state.timeInt) * sinc(m_state.timeNow - (double)j);
      }
      data_out = temp1;
    } else {
      one_over_factor = 1.0 / factor;
      for (int j = left_limit; j < right_limit; j++) {
        temp1 += getData(j - m_state.timeInt) * one_over_factor *
                 sinc(one_over_factor * (m_state.timeNow - (double)j));
      }
      data_out = temp1;
    }
    ...
    if (samplesOut) {
      ...
      // left_limit, right_limit, m_state.nSamplesTotal, left_limit -
      // m_state.timeInt, right_limit - m_state.timeInt - 1);
...
void GGWave::decode_variable() {
  ...
  if (m_rx.historyId == 0 || m_rx.receiving) {
    ...
    // calculate spectrum
    FFT(m_rx.amplitudeAverage.data(), m_rx.fftOut.data(), m_samplesPerFrame,
        m_rx.fftWorkI.data(), m_rx.fftWorkF.data());

    for (int i = 0; i < m_samplesPerFrame; ++i) {
      m_rx.spectrum[i] = (m_rx.fftOut[2 * i + 0] * m_rx.fftOut[2 * i + 0] +
                          m_rx.fftOut[2 * i + 1] * m_rx.fftOut[2 * i + 1]);
    }
  ...
  if (m_rx.framesLeftToRecord > 0) {
    memcpy(m_rx.amplitudeRecorded.data() +
               (m_rx.framesToRecord - m_rx.framesLeftToRecord) *
                   m_samplesPerFrame,
           m_rx.amplitude.data(), m_samplesPerFrame * sizeof(float));
  ...
  if (m_rx.analyzing) {
    ...
    bool isValid = false;
    for (int protocolId = 0; protocolId < (int)m_rx.protocols.size();
         ++protocolId) {
      ...
      // note : not sure if looping backwards here is more meaningful than
      // looping forwards
      for (int ii = m_nMarkerFrames * stepsPerFrame - 1; ii >= 0; --ii) {
        ...
        for (int itx = 0; itx < 1024; ++itx) {
          int offsetTx =
              offsetStart + itx * protocol.framesPerTx * stepsPerFrame;
          if (offsetTx >= m_rx.recvDuration_frames * stepsPerFrame ||
              (itx + 1) * protocol.bytesPerTx >= (int)m_dataEncoded.size()) {
            break;
          }
          ...
          // note : should we skip the first and last frame here as they are
          // amplitude-smoothed?
          for (int k = 1; k < protocol.framesPerTx; ++k) {
            for (int i = 0; i < m_samplesPerFrame; ++i) {
              m_rx.fftOut[i] +=
                  m_rx.amplitudeRecorded[(offsetTx + k * stepsPerFrame) * step +
                                         i];
            }
          }

          FFT(m_rx.fftOut.data(), m_samplesPerFrame, m_rx.fftWorkI.data(),
              m_rx.fftWorkF.data());

          for (int i = 0; i < m_samplesPerFrame; ++i) {
            m_rx.spectrum[i] =
                (m_rx.fftOut[2 * i + 0] * m_rx.fftOut[2 * i + 0] +
                 m_rx.fftOut[2 * i + 1] * m_rx.fftOut[2 * i + 1]);
          }
          for (int i = 1; i < m_samplesPerFrame / 2; ++i) {
            m_rx.spectrum[i] += m_rx.spectrum[m_samplesPerFrame - i];
          ...
          if (itx * protocol.bytesPerTx > m_encodedDataOffset &&
              knownLength == false) {
            RS::ReedSolomon rsLength(1, m_encodedDataOffset - 1,
                                     m_workRSLength.data());
            if ((rsLength.Decode(m_dataEncoded.data(), m_rx.data.data()) ==
                 0) &&
                (m_rx.data[0] > 0 && m_rx.data[0] <= 140)) {
              knownLength = true;
              ...
              const int nTotalBytesExpected =
                  m_encodedDataOffset + decodedLength +
                  ::getECCBytesForLength(decodedLength);
              const int nTotalFramesExpected =
                  2 * m_nMarkerFrames +
                  ((nTotalBytesExpected + protocol.bytesPerTx - 1) /
                   protocol.bytesPerTx) *
                      protocol.framesPerTx;
              if (m_rx.recvDuration_frames > nTotalFramesExpected ||
                  m_rx.recvDuration_frames <
                      nTotalFramesExpected - 2 * m_nMarkerFrames) {
...

          {
            const int nTotalBytesExpected =
                ...
                ::getECCBytesForLength(decodedLength);
            if (knownLength &&
                itx * protocol.bytesPerTx > nTotalBytesExpected + 1) {
              break;
            }
          }
        }
        ...
        if (knownLength) {
          RS::ReedSolomon rsData(decodedLength,
                                 ::getECCBytesForLength(decodedLength),
                                 m_workRSData.data());

          if (rsData.Decode(m_dataEncoded.data() + m_encodedDataOffset,
                            m_rx.data.data()) == 0) {
            if (decodedLength > 0) {
              if (m_isDSSEnabled) {
  ...
  if (m_rx.receiving == false) {
    ...
    for (int i = 0; i < m_rx.protocols.size(); ++i) {
      ...
      for (int i = 0; i < m_nBitsInMarker; ++i) {
        ...
        if (i % 2 == 0) {
          if (m_rx.spectrum[bin] <=
              m_soundMarkerThreshold * m_rx.spectrum[bin + m_freqDelta_bin])
            --nDetectedMarkerBits;
        } else {
          if (m_rx.spectrum[bin] >=
              m_soundMarkerThreshold * m_rx.spectrum[bin + m_freqDelta_bin])
    ...
    if (isReceiving) {
      ...
      m_rx.recvDuration_frames =
          ...
          maxFramesPerTx(m_rx.protocols, true) *
              ((kMaxLengthVariable +
                ::getECCBytesForLength(kMaxLengthVariable)) /
                   minBytesPerTx(m_rx.protocols) +
               1);
...
void GGWave::decode_fixed() {
  ...
  // calculate spectrum
  FFT(m_rx.amplitude.data(), m_rx.fftOut.data(), m_samplesPerFrame,
      m_rx.fftWorkI.data(), m_rx.fftWorkF.data());
  ...
  for (int i = 0; i < m_samplesPerFrame; ++i) {
    m_rx.spectrumHistoryFixed[m_rx.historyIdFixed][i] =
        GG_MIN(255.0f, GG_MAX(0.0f, (float)round(m_rx.spectrum[i] * amax)));
  }

  // float -> uint16_t
  // amax = 65535.0f/(amax == 0.0f ? 1.0f : amax);
  // for (int i = 0; i < m_samplesPerFrame; ++i) {
  //    m_rx.spectrumHistoryFixed[m_rx.historyIdFixed][i] = GG_MIN(65535.0f,
  //    GG_MAX(0.0f, (float) round(m_rx.spectrum[i]*amax)));
  //}
  ...
  for (int protocolId = 0; protocolId < (int)m_rx.protocols.size();
    ...
    const int totalLength =
        m_payloadLength + getECCBytesForLength(m_payloadLength);
    const int totalTxs =
        protocol.extra *
        ((totalLength + protocol.bytesPerTx - 1) / protocol.bytesPerTx);
    ...
    for (int k = 0; k < totalTxs; ++k) {
      ...
      for (int i = 0; i < protocol.framesPerTx; ++i) {
        ...
        for (int j = 0; j < protocol.bytesPerTx; ++j) {
          int f0bin = 0;
          auto f0max =
              m_rx.spectrumHistoryFixed[historyId][binStart + 2 * j * binDelta];

          for (int b = 1; b < 16; ++b) {
            {
              const auto &v =
                  m_rx.spectrumHistoryFixed[historyId]
                                           [binStart + 2 * j * binDelta + b];
          ...
          int f1bin = 0;
          if (protocol.extra == 1) {
            auto f1max = m_rx.spectrumHistoryFixed[historyId][binStart +
                                                              2 * j * binDelta +
                                                              binOffset];
            for (int b = 1; b < 16; ++b) {
              const auto &v =
                  m_rx.spectrumHistoryFixed[historyId][binStart +
                                                       2 * j * binDelta +
                                                       binOffset + b];
          ...
          if ((k + 0) % protocol.extra == 0)
            m_rx.detectedTones[(2 * j + 0) * 16 + f0bin]++;
          if ((k + 1) % protocol.extra == 0)
            m_rx.detectedTones[(2 * j + 1) * 16 + f1bin]++;
        }
      }

      if (protocol.extra > 1 && (k % protocol.extra == 0))
        continue;

      int txNeeded = 0;
      int txDetected = 0;
      for (int j = 0; j < protocol.bytesPerTx; ++j) {
        if ((k / protocol.extra) * protocol.bytesPerTx + j >= totalLength)
          break;
        txNeeded += 2;
        for (int b = 0; b < 16; ++b) {
          if (m_rx.detectedTones[(2 * j + 0) * 16 + b] >
              protocol.framesPerTx / 2) {
            m_rx.detectedBins[2 * ((k / protocol.extra) * protocol.bytesPerTx +
                                   j) +
                              0] = b;
            txDetected++;
          }
          if (m_rx.detectedTones[(2 * j + 1) * 16 + b] >
              protocol.framesPerTx / 2) {
            m_rx.detectedBins[2 * ((k / protocol.extra) * protocol.bytesPerTx +
                                   j) +
                              1] = b;
    ...
    if (detectedSignal) {
      RS::ReedSolomon rsData(m_payloadLength,
                             getECCBytesForLength(m_payloadLength),
                             m_workRSData.data());

      for (int j = 0; j < totalLength; ++j) {
        m_dataEncoded[j] =
            (m_rx.detectedBins[2 * j + 1] << 4) + m_rx.detectedBins[2 * j + 0];
      }
