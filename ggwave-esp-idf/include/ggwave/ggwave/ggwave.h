...
//
//   If payloadLength <= 0, then GGWave will transmit with variable payload
//   length depending on the provided payload. Sound markers are used to
//   identify the start and end of the transmission.
//
...
//
//   The operatingMode controls which functions of the ggwave instance are
//   enabled. Use this parameter to reduce the memory footprint of the ggwave
//   instance. For example, if only Rx is enabled, then the memory buffers
//   needed for the Tx will not be allocated.
//
typedef struct {
...
//
//   Intentionally passing it as void * instead of FILE * to avoid including a
//   header
//
...
//   waveformBuffer - the generated audio waveform. must be big enough to fit
//   the generated data query          - if == 0, encode data in to
//   waveformBuffer, returns number of bytes
...
//
//   This function can be used to encode some binary data (payload) into an
//   audio waveform.
//
//     payload -> waveform
//
//   When calling it, make sure that the waveformBuffer is big enough to store
//   the generated waveform. This means that its size must be at least:
//
//     nSamples*sizeOfSample_bytes
//
//   Where nSamples is the number of audio samples in the waveform and
//   sizeOfSample_bytes is the size of a single sample in bytes based on the
//   sampleFormatOut parameter specified during the initialization of the GGWave
//   instance.
//
//   If query != 0, then this function does not perform the actual encoding and
//   just outputs the expected size of the waveform that would be generated if
//   you call it with query == 0. This mechanism can be used to ask ggwave how
//   much memory to allocate for the waveformBuffer. For example:
//
//     // this is the data to encode
//     const char * payload = "test";
//
//     // query the number of bytes in the waveform
//     int n = ggwave_encode(instance, payload, 4, GGWAVE_PROTOCOL_AUDIBLE_FAST,
//     25, NULL, 1);
//
...
//     ggwave_encode(instance, payload, 4, GGWAVE_PROTOCOL_AUDIBLE_FAST, 25,
//     waveform, 0);
//
//   The payloadBuffer can be any binary data that you would like to transmit
//   (i.e. the payload). Usually, this is some text, but it can be any sequence
//   of bytes.
//
GGWAVE_API int ggwave_encode(ggwave_Instance instance,
                             const void *payloadBuffer, int payloadSize,
                             ggwave_ProtocolId protocolId, int volume,
                             void *waveformBuffer, int query);
...
//
//   Use this function to continuously provide audio samples to a GGWave
//   instance. On each call, GGWave will analyze the provided data and if it
//   detects a payload, it will return a non-zero result.
//
//     waveform -> payload
//
//   If the return value is -1 then there was an error during the decoding
//   process. Usually can occur if there is a lot of background noise in the
//   audio.
//
//   If the return value is greater than 0, then there are that number of bytes
//   decoded.
//
//   IMPORTANT:
//   Notice that the decoded data written to the payloadBuffer is NOT null
//   terminated.
//
//   Example:
//
...
//
//         int ret = ggwave_decode(instance, waveform,
//         samplesPerFrame*sizeOfSample_bytes, payload); if (ret > 0) {
//             payload[ret] = 0; // null terminate the string
//             printf("Received payload: '%s'\n", payload);
//         }
//     }
//
GGWAVE_API int ggwave_decode(ggwave_Instance instance,
...
//
//   If the return value is -2 then the provided payloadBuffer was not big
//   enough to store the decoded data.
//
//   See ggwave_decode for more information
//
GGWAVE_API int ggwave_ndecode(ggwave_Instance instance,
                              const void *waveformBuffer, int waveformSize,
...
//
//   If an Rx protocol is enabled, newly constructued GGWave instances will
//   attempt to decode received data using this protocol. By default, all
//   protocols are enabled. Use this function to restrict the number of Rx
//   protocols used in the decoding process. This helps to reduce the number of
//   false positives and improves the transmission accuracy, especially when the
//   Tx/Rx protocol is known in advance.
//
//   Note that this function does not affect the decoding process of instances
//   that have already been created.
//
GGWAVE_API void ggwave_rxToggleProtocol(ggwave_ProtocolId protocolId,
                                        int state);
...
//   state      - 0 - disable, 1 - enable
//
//   If an Tx protocol is enabled, newly constructued GGWave instances will be
//   able to transmit data using this protocol. By default, all protocols are
//   enabled. Use this function to restrict the number of Tx protocols used for
//   transmission. This can reduce the required memory by the GGWave instance.
//
//   Note that this function does not affect instances that have already been
//   created.
//
GGWAVE_API void ggwave_txToggleProtocol(ggwave_ProtocolId protocolId,
                                        int state);

// Set freqStart for an Rx protocol
GGWAVE_API void ggwave_rxProtocolSetFreqStart(ggwave_ProtocolId protocolId,
                                              int freqStart);

// Set freqStart for a Tx protocol
GGWAVE_API void ggwave_txProtocolSetFreqStart(ggwave_ProtocolId protocolId,
                                              int freqStart);

// Return recvDuration_frames value for a rx protocol
GGWAVE_API int ggwave_rxDurationFrames(ggwave_Instance instance);
...
template <typename T> struct ggvector {
private:
  T *m_data;
  int m_size;
...
template <typename T> struct ggmatrix {
private:
  T *m_data;
  int m_size0;
  int m_size1;
...
public:
  ...
  ggmatrix(T *data, int size0, int size1)
      : m_data(data), m_size0(size0), m_size1(size1) {}
...
#include <stdio.h>
    ...
    defined(ARDUINO_ARDUINO_NANO33BLE) || defined(ARDUINO_ARCH_MBED_RP2040) || \
...
public:
  ...
  struct Protocols {
    ...
    int size() const { return GGWAVE_PROTOCOL_COUNT; }

    bool empty() const { return size() == 0; }

    Protocol &operator[](ProtocolId id) { return data[id]; }

    Protocol &operator[](int id) { return data[id]; }

    const Protocol &operator[](ProtocolId id) const { return data[id]; }

    const Protocol &operator[](int id) const { return data[id]; }
...
// For Arduino Uno, we put the strings in PROGMEM to save as much RAM as
// possible:
...
#ifndef GGWAVE_CONFIG_FEW_PROTOCOLS
        protocols.data[GGWAVE_PROTOCOL_AUDIBLE_NORMAL] = {
            GGWAVE_PSTR("Normal"), 40, 9, 3, 1, true,
        };
        protocols.data[GGWAVE_PROTOCOL_AUDIBLE_FAST] = {
            GGWAVE_PSTR("Fast"), 40, 6, 3, 1, true,
        };
        protocols.data[GGWAVE_PROTOCOL_AUDIBLE_FASTEST] = {
            GGWAVE_PSTR("Fastest"), 40, 3, 3, 1, true,
        };
        protocols.data[GGWAVE_PROTOCOL_ULTRASOUND_NORMAL] = {
            GGWAVE_PSTR("[U] Normal"), 320, 9, 3, 1, true,
        };
        protocols.data[GGWAVE_PROTOCOL_ULTRASOUND_FAST] = {
            GGWAVE_PSTR("[U] Fast"), 320, 6, 3, 1, true,
        };
        protocols.data[GGWAVE_PROTOCOL_ULTRASOUND_FASTEST] = {
            GGWAVE_PSTR("[U] Fastest"), 320, 3, 3, 1, true,
        };
#endif
        protocols.data[GGWAVE_PROTOCOL_DT_NORMAL] = {
            GGWAVE_PSTR("[DT] Normal"), 24, 9, 1, 1, true,
        };
        protocols.data[GGWAVE_PROTOCOL_DT_FAST] = {
            GGWAVE_PSTR("[DT] Fast"), 24, 6, 1, 1, true,
        };
        protocols.data[GGWAVE_PROTOCOL_DT_FASTEST] = {
            GGWAVE_PSTR("[DT] Fastest"), 24, 3, 1, 1, true,
        };
        protocols.data[GGWAVE_PROTOCOL_MT_NORMAL] = {
            GGWAVE_PSTR("[MT] Normal"), 24, 9, 1, 2, true,
        };
        protocols.data[GGWAVE_PROTOCOL_MT_FAST] = {
            GGWAVE_PSTR("[MT] Fast"), 24, 6, 1, 2, true,
        };
        protocols.data[GGWAVE_PROTOCOL_MT_FASTEST] = {
            GGWAVE_PSTR("[MT] Fastest"), 24, 3, 1, 2, true,
        };
...
#undef GGWAVE_PSTR
  ...
  //
  //   If the protocol is mono-tone, each element of the vector corresponds to a
  //   single tone. Otherwise, the tones within a single Tx are separated by
  //   value of -1
  //
  ...
  //
  //   All memory buffers used by the GGWave instance are allocated with this
  //   function. No memory allocations occur after that.
  //
  //   Call this method if you used the default constructor.
  //   Do not call this method if you used the constructor with parameters.
  //
  //   The encode() and decode() methods will not work until this method is
  //   called.
  //
  //   The sizes of the buffers are determined by the parameters and the
  //   contents of:
  //
  //     - GGWave::Protocols::rx()
  //     - GGWave::Protocols::tx()
  //
  //   For optimal performance and minimum memory usage, make sure to enable
  //   only the Rx and Tx protocols that you need.
  //
  ...
  //     instance.encode();
  //
  //   The created instance will only be able to transmit data using the
  //   "Normal" protocol. Rx will be disabled.
  //
  ...
  //     instance.decode(...);
  //
  //   If "allocate" is false, the memory buffers are not allocated and only the
  //   required size is computed. This is useful if you want to just see how
  //   much memory is needed for the specific set of parameters and protocols.
  //   Do not use this function after you have already prepared the instance.
  //   Instead, use the heapSize() method to see how much memory is used.
  //
  ...
  //
  //   Returns false upon invalid parameters or failure to initialize the
  //   transmission
  //
  bool init(const char *text, TxProtocolId protocolId,
            const int volume = kDefaultVolume);
  bool init(int dataSize, const char *dataBuffer, TxProtocolId protocolId,
            const int volume = kDefaultVolume);

  // Expected waveform size of the encoded Tx data in bytes
  //
  //   When the output sampling rate is not equal to operating sample rate the
  //   result of this method is overestimation of the actual number of bytes
  ...
  //
  //   When the output sampling rate is not equal to operating sample rate the
  //   result of this method is overestimation of the actual number of samples
  //   that would be produced
  //
  ...
  //
  //   After calling this method, use the Tx methods to get the encoded audio
  //   data.
  //
  ...
  //
  //   The samples pointed to by "data" should be in the format given by
  //   sampleFormatInp(). After calling this method, use the Rx methods to check
  //   if any data was decoded successfully.
  //
  ...
  //
  //   Call this method after calling encode() to get the generated waveform.
  //   The format of the samples pointed to by the returned pointer is
  //   determined by the sampleFormatOut() method.
  //
  ...
  // The instance will allow Tx only with these protocols. They are determined
  // upon construction or when calling the prepare() method, base on the
  // contents of the global GGWave::Protocols::tx()
  ...
  // They are determined upon construction or when calling the prepare() method,
  // base on the contents of the global GGWave::Protocols::rx()
  //
  // Note: do not enable protocols that were not enabled upon preparation of the
  // GGWave instance, or the decoding will likely crash
  //
  ...
  //   This will initialize some internal coefficients and store them in wi and
  //   wf for future usage.
  //
  ...
  //
  static int computeFFTR(const float *src, float *dst, int N, int *wi,
                         float *wf);
  ...
  //
  static int filter(ggwave_Filter filter, float *waveform, int N, float p0,
                    float p1, float *w);

  // Resample audio waveforms from one sample rate to another using sinc
  // interpolation
  class Resampler {
  public:
    ...
    int resample(float factor, int nSamples, const float *samplesInp,
                 float *samplesOut);
  ...
  private:
    ...
    // this defines how finely the sinc function is sampled for storage in the
    // table
