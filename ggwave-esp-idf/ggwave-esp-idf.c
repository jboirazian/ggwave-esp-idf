...
ggwave_esp_idf_config_t ggwave_esp_idf_default_config(void) {
    ggwave_esp_idf_config_t config = {
        .payloadLength = 16,
        .sampleRate = 24000,
        .samplesPerFrame = 512,
        .sampleFormatInp = GGWAVE_SAMPLE_FORMAT_I16,
        .operatingMode = GGWAVE_OPERATING_MODE_RX | GGWAVE_OPERATING_MODE_USE_DSS
    };
    return config;
}
...
int ggwave_esp_idf_decode(ggwave_Instance instance,
    ...
    return ggwave_ndecode(instance, waveformBuffer, waveformSize, payloadBuffer, payloadSize);
}